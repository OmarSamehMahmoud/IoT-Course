...
#include "esp_https_server.h"            // HTTPS server
#include "esp_err.h"                    // error codes

#include "certs/server_certs.h"
// ======================
// CONFIGURATION
// ======================
...
static void start_https_server(void)
{
    ...
    conf.port_secure = 443;  // HTTPS port

    // Embed certificates
    conf.cacert_pem = (uint8_t*)server_cert_pem;
    conf.cacert_len = strlen(server_cert_pem);
    conf.prvtkey_pem = (uint8_t*)server_key_pem;
    conf.prvtkey_len = strlen(server_key_pem);

    if (httpd_ssl_start(&http_server, &conf) == ESP_OK) {
        ...
        httpd_uri_t api_uri = { .uri = "/api", .method = HTTP_GET, .handler = api_handler };
...
void app_main(void)
{
    ...
    // 6) Start HTTP server
	start_https_server();  // Use HTTPS instead;
